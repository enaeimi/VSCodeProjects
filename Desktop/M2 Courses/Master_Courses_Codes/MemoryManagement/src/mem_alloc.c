...
char *AllocateMemoryBlock(char* pointer,int size) {
	free_block_t fb = (free_block_t)((char*)pointer + size + sizeof(busy_block_s));
	fb->size = pointer->size - (size + sizeof(busy_block_s));//The size of new free block (after allocation) is equal to size of block which referred by "pointer" - reuquested size + size of descriptor
	fb->next = pointer->next;//next block of new free block is the same as next of "pointer" block
	busy_block_t bb = (busy_block_t)pointer;
	bb->size = size + sizeof(busy_block_s);

	//Resetting next pointer of previous block 
	free_block_t tmpPointer = first_free;
	while (tmpPointer != NULL && tmpPointer->next!=pointer ) {
		tmpPointer=tmpPointer->next;
	}
	if(tmpPointer==NULL) { //Allocated space is in the beginning of memory block
		first_free = fb;
	}
	else {
		tmpPointer->next = fb;
...
char *FindWorstFit(int size) {
 	...
 	free_block_t smallest_block,tmpblock = first_free;
 	int smallest_size = smallest_block->size - size;
	tmpblock=tmpblock->next;
	while (smallest_block) {
		if () {
